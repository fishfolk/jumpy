...
NOTE: To get the hitbox of a Player node, use `Player::get_hitbox(node_handle)`, so that you get the correct size if the player node, for example, is in a crouched state.
