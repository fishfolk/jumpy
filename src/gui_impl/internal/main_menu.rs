...
use ff_core::gui::{
    gui_context, GuiContext, Menu, MenuEntry, MenuResult, Panel, WINDOW_BG_COLOR, WINDOW_MARGIN_H,
    WINDOW_MARGIN_V,
};
...
impl MainMenuState {
    ...
    fn draw_current(&mut self, ctx: &GuiContext) -> Option<MainMenuResult> {
        if !matches!(
            self.current_level,
        ...
        if let Some(instance) = &mut self.current_instance {
            if let Some(res) = instance.ui(ctx) {
...
impl GameState for MainMenuState {
    ...
    fn draw(&mut self, _delta_time: f32) -> Result<()> {
        let _needs_repaint = gui_context().run(window(), |ctx| {
