...
pub fn hydrate(
    ...
    for spawner_ent in spawner_entities {
        ...
        if let BuiltinElementKind::FishSchool {
            kinds,
            base_count,
            count_variation,
            spawn_range,
        ...
        } = &element_meta.builtin
        {
            ...
            element_handles.insert(fish_school_ent, element_handle.clone());
            ...
            for _ in 0..fish_count {
                ...
                atlas_sprites.insert(fish_ent, AtlasSprite::new(atlas.clone()));
                animated_sprites.insert(
                    fish_ent,
                    AnimatedSprite {
                        frames: Arc::new([0, 1, 2, 3]),
                        fps: 6.0,
                        repeat: true,
                        ..default()
                    },
...
pub fn update_fish_schools(
    ...
    for (school_ent, school) in entities.iter_with(&fish_schools) {
        ...
        let BuiltinElementKind::FishSchool {
            school_size,
            ..
        } = &element_meta.builtin
        else { continue };
        ...
        for fish_ent in school.fish.iter() {
            ...
            match &fish.state {
                FishState::Moving { from, to } => {
                    ...
                    sprite.index = (4.0 * lerp_progress).floor() as usize;
