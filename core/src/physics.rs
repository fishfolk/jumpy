...
use crate::math::{deg_to_rad, vec2, Rect, Size, Vec2};
use crate::render::draw_rectangle_outline;
...
pub fn fixed_update_rigid_bodies(
    ...
    for (_, (transform, body)) in world.query_mut::<(&mut Transform, &mut RigidBody)>() {
        ...
        if body.can_rotate {
            apply_rotation(delta_time, transform, &mut body.velocity, 0.0, false);
        }
    }
...
fn apply_rotation(
    ...
    if !angular_velocity.is_zero() {
        transform.rotation += deg_to_rad(angular_velocity * delta_time);
