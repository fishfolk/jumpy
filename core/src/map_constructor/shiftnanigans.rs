    ...
    pub fn new(map_size: UVec2, tile_size: Vec2, tile_layers: &Vec<TileLayer>, element_layers: &Vec<ElementLayer>) -> ShiftnanigansMapConstructor {
        ...
        for x in 0..map_size.x {
            ...
            for y in 0..map_size.y {
                let ungrouped_pixel_option = UngroupedPixel::from_layers_and_location(tile_layers, element_layers, x, y);
        ...
        'get_top_height: {
            for y in 0..map_size.y as usize {
                ...
                for x in 0..map_size.x as usize {
                    if ungrouped_pixel_per_y_per_x[x][y].is_none() {
                        top_height = y;
        ...
        let right_width;
        'get_right_width: {
            for x in (0..map_size.x as usize).rev() {
                for y in 0..map_size.y as usize {
                    if ungrouped_pixel_per_y_per_x[x][y].is_none() {
...
impl UngroupedPixel {
    fn from_layers_and_location(tile_layers: &Vec<TileLayer>, element_layers: &Vec<ElementLayer>, x: u32, y: u32) -> Option<Self> {
