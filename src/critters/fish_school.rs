...
pub fn spawn_fish_school(world: &mut World, spawn_position: Vec2) -> Result<Entity> {
    let fish_school_icon_texture = get_texture(FISH_SCHOOL_ICON_TEXTURE_ID);
    let fish_school_icon_texture_size = fish_school_icon_texture.size();
    ...
    let mut fish_school = FishSchool {
        spawn_pos: spawn_position + fish_school_icon_texture_size.as_vec2() / 2.0,
    ...
    for _ in 0..fish_count {
        ...
        let texture = get_texture(FISH_TEXTURE_IDS[texture_index]);
        ...
        let fish_entity = world.spawn((
            Fish {
                ...
                state_timer: Timer::from_secs_f32(rand::gen_range(0.2, 1.0)),
            },
    ...
    for school in &schools {
        ...
        for fish_entity in &school.fish_entities {
            ...
            let rect = Rect::new(
                pos.x - padding / 2.0,
                pos.y - padding / 2.0,
                sprite_size.width + padding,
                sprite_size.height + padding,
            );
            ...
            let rand_bool = || rand::gen_range(0u8, 2) > 0;
            let rand_delay = |min, max| Timer::from_secs_f32(rand::gen_range(min, max));
...
pub fn debug_draw_fish_schools(world: &mut World) {
    for (_, school) in world.query::<&FishSchool>().iter() {
        ...
        draw_rectangle_outline(
            ...
            school.bounds_rect.width,
            school.bounds_rect.height,
            1.0,
            if school.is_grouped {
...
fn get_school_info(world: &World, school: &FishSchool) -> Option<SchoolInfo> {
    let fish_texture = get_texture(FISH_TEXTURE_IDS[0]);
    let fish_texture_size = fish_texture.size();
    ...
    for transform in &fish_transforms {
        ...
        school_bounds_max.x = school_bounds_max.x.max(pos.x + fish_texture_size.width);
        school_bounds_max.y = school_bounds_max.y.max(pos.y + fish_texture_size.height);
    }
